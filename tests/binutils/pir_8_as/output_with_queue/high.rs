...
fn wait_for_label_write_no() {
    ...
    output.wait_for_label("owo".to_string(), 0, LabelFragment::High);
    output.write_all(&['U' as u8, 'w' as u8, 'U' as u8], &BTreeMap::new()).unwrap();
    output.wait_for_label("eWe".to_string(), 0, LabelFragment::High);
...
fn wait_for_label_write_yes() {
    ...
    output.wait_for_label("owo".to_string(), 1, LabelFragment::High);
    ...
    assert_eq!(&dest, &[0x01, 'U' as u8, 'w' as u8, 'U' as u8]);
}
