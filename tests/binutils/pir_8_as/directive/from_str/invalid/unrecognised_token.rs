...
fn label_fragment() {
    ...
    for op in &["load", "load-offset"] {
        for pad in 1..5 {
            unrecognised_token(&format!("label{e:w$}{}", op, e = "", w = pad),
                               TOKENS_FRAGMENT,
                               1..10,
                               |len, _| ParseInstructionError::UnrecognisedToken(len, TOKENS_FRAGMENT));
        }
    }
}
...
fn label_offset() {
    ...
    for addr in (0x1FFFF..=0x4FFFF).step_by(0xFF) {
        assert_eq!(AssemblerDirective::from_str(&format!(":label load-offset full UwU -{}", addr)),
                   Err(ParseInstructionError::UnrecognisedToken(29, TOKENS_OFFSET)),
                   "{:#x}",
                   addr);
        assert_eq!(AssemblerDirective::from_str(&format!(":label load-offset full UwU -{:#0x}", addr)),
                   Err(ParseInstructionError::UnrecognisedToken(29, TOKENS_OFFSET)),
                   "{:#x}",
                   addr);
        assert_eq!(AssemblerDirective::from_str(&format!(":label load-offset full UwU -{:#0X}", addr)),
                   ...
                   "{:#x}",
                   addr);
        assert_eq!(AssemblerDirective::from_str(&format!(":label load-offset full UwU -{:#0o}", addr)),
                   Err(ParseInstructionError::UnrecognisedToken(29, TOKENS_OFFSET)),
                   "{:#x}",
                   addr);
        assert_eq!(AssemblerDirective::from_str(&format!(":label load-offset full UwU -{:#0b}", addr)),
                   Err(ParseInstructionError::UnrecognisedToken(29, TOKENS_OFFSET)),
                   "{:#x}",
                   addr);
